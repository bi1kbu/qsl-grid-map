...
11. 本台网格位置显示由后台开关控制；开启时按 `defaultCenterGrid（默认中心网格）` 绘制独立虚线边框，边框颜色由后台配置，点击后才显示“本台网格”提示。
...
2. `showHomeGrid（显示本台网格位置）`：必填，默认开启；关闭后不在地图上凸显本台网格位置。
3. `homeGridBorderColor（本台网格边框颜色）`：必填，`#RRGGBB` 格式，默认 `#dc2626`。
4. `defaultZoom（默认缩放级别）`：必填，范围 `1` 到 `18`，默认 `3`。
5. `minZoom（最小缩放级别）`：必填，范围 `1` 到 `18`，默认 `3`。
6. `maxZoom（最大缩放级别）`：必填，范围 `1` 到 `18`，默认 `8`。
7. 配置值由 Halo 插件设置表单保存，服务端通过官方 `ReactiveSettingFetcher.fetch("map", MapViewSettings.class)` 读取。
8. 服务端对中心网格、显示开关、边框颜色和缩放级别做兜底规范化，未设置显示开关时默认开启，非法中心网格回退到 `OM89`，非法边框颜色回退到 `#dc2626`，非法缩放限制在 `1` 到 `18`；若最小缩放大于最大缩放，则回退默认 `3~8`。
...
13. 前台按 `showHomeGrid（显示本台网格位置）` 决定是否显示本台网格虚线边框；开启时颜色由 `homeGridBorderColor（本台网格边框颜色）` 控制，点击边框后弹窗显示“本台网格”。
