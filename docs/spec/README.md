...
运行联动状态：`qsl-management` 公开网格接口匿名访问已通过；本插件页面接口匿名公开访问已通过，浏览器页面固定请求 `sceneType=QSO` 的数据接口并返回 200。当前已配置天地图应用 key，天地图矢量底图与注记瓦片均返回 200，默认中心网格为 `OM89`、默认缩放级别为 `3`，缩放范围由后台配置控制，数据加载后不再自动 `fitBounds（适配范围）` 覆盖后台默认视图；本台网格可通过后台开关控制是否显示，开启时使用后台可配置颜色的虚线边框标识，点击后弹窗显示本台网格名称。默认视图截图文件：`tmp/qsl-grid-map-default-view-om89-no-fitbounds.png`。截图已确认页面仅保留全窗口地图面板，无筛选面板、右侧清单或前台缩放测试控件。编辑器 UI 扩展已生成 `src/main/resources/ui/main.js` 与 `src/main/resources/ui/style.css`，用于在 `/` 菜单中插入 QSL 通联网格地图预览卡片，截图文件：`tmp/qsl-grid-map-editor-preview-card.png`。
